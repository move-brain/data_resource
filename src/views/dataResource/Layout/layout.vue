...
et.on("handleclikemenu",async (e)=>{
  ...
  if (e.id!=infoid) { 
    await getmainmes(e.id)}   //使用await语法糖  实现js异步
      router.push({name:'poc',params:{id:Directoryinfo.info.data.currentCatalogueBasicInfor.id}})   //此为动态路由,已当前显示的目录的id作为参数
}) 
...
router.push({name:'poc',params:{id:Directoryinfo.info.data.currentCatalogueBasicInfor.id}})
})
  //在点击了数据字段表格触发的事件
  et.on('cellclick',(e)=>{
    ...
    getdatafield(e.id).then(async res=>{
     ...
     router.push({name:'fielddata',params:{id:res.data.data.currentFieldBasicInfor.id}})
    })
  })
//点击了父 子 类  别的组件触发的事件
//非数据字段表格的点击事件
