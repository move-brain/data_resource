...
let fa = "[]";
let currentCatalogueBasicInfor =
  data.currentCatalogueBasicInfor != null
    ? data.currentCatalogueBasicInfor
    : null;
let childCatalogues =
  data.childCatalogues && data.childCatalogues.lenght != 0
    ? data.childCatalogues
    : null;
...
const title = ref(
  currentCatalogueBasicInfor.title +
    "（ID-" +
    currentCatalogueBasicInfor.id +
    "）"
);
//判断是否有父表格
let faform = reactive([]);
...
<template>
  ...
  <div class="container">
    <h2 class="font-semibold text-2xl my-5">{{ title }}</h2>
    <p>{{ description }}</p>
  </div>
  <classdata
    v-if="parentId != null && fa != '[]'"
    :form="faform"
    formtype="f"
  />
