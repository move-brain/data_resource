...
<div  class="w-[70%] h-[75%] relative top-[5%] max-h-[400px]  max-w-[1100px]">
    <div class="h-[12vh] flex justify-center items-center text-2xl font-bold " ><span >忘记密码</span></div>
    <div class="bg-[#fff] w-[100%] h-fit  rounded-xl flex  flex-col justify-start items-center pb-[3%]">
...
<el-form-item  class="w-[100%] flex justify-between items-center">
    <el-input placeholder="请输入邮箱验证码" v-model="form.emailcode" class="havebutton"  />
...
<div class="w-[100%] text-[#989d9e]" ><span>安全强度：{{safe_strength }}</span></div>
<div  class="w-[100%] text-[#989d9e] my-2"> <el-progress stroke-width="8" :percentage="progress" show-text="false" text-inside="true" /></div>
<div class="allgo w-[100%] flex justify-start flex-col  items-start text-[#989d9e]" >
<div ><svgicon  iconName="icon-kongxinduigou" color="#989d9e" ></svgicon>最少8个字符</div>
<div><svgicon  iconName="icon-kongxinduigou" color="#989d9e" ></svgicon>不能全为数字、字母或特殊符号</div>
<div><svgicon  iconName="icon-kongxinduigou" color="#989d9e" ></svgicon>数字、字母、特殊字符任意组合</div>
</div>
</div>
</template>
<el-input @blur="isblur" @focus="isfocus" placeholder="请输入新密码" @input="inpass" v-model="form.password" show-password />     
</el-tooltip>
...
<span @click="gologin"  class="mt-[2.5%] text-xs text-[#158aa2] cursor-pointer" >返回登录</span>
    </div>
</div>
</div>
</template>
